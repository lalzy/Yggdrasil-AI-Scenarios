...
public class ChatService (AppDbContext db)
{
    ...
    ///<summary>Update the time of conversation/world</summary>
    private void UpdateTimes(Conversation conversation){
        var world = _db.Set<World>().FirstOrDefault(w => w.ID == conversation!.World_ID);
        var currentTime = DateTime.UtcNow;
        conversation.UpdatedAt = currentTime;
        world!.LastUsed = currentTime;

    }

    private void UpdateTimes(Guid conversation_ID){
        UpdateTimes(_db.Set<Conversation>().FirstOrDefault(c => c.ID == conversation_ID)!);
    }
    ...
    public ServiceResult<ChatMessage> Create(ChatMessageRequest request){
        var conversation = _db.Set<Conversation>().FirstOrDefault(c => c.ID == request.Conversation_ID);
    ...
    public ServiceResult<ChatMessage> Update(Guid message_ID, ChatMessageUpdateRequest request){
        ...
        if(request.Content != null){
            ...
            UpdateTimes(message.Conversation_ID);
